...
module.exports = {
  ...
  module: {
    //处理项目中的不同的模块
    rules: [
      //格式array,创建模块时，匹配请求的规则数组。这些规则能够对修改模块的创建方式。这些规则能够对（module）应用loader，或修改解析器（parser）
      {
        //本json是对js的eslint的检查
  ...
  plugins: [
    ...
    new HtmlWebpackPlugin({
      //简化了html文件的创建，以便为webpack包提供服务。
      filename: resolve('/dist/index.html'), //处理dirname路径的问题 ，这里等同于'../dist/index.html'
      template: './src/pages/test/index.js',
      chunks: ['common'] //选择加载的css和js,模块名对应上面entry接口的名称
    }),
    // new HtmlWebpackPlugin({
    ...
    new ExtractTextPlugin({
      //从bundle中提取出
      filename: getPath => {
        return getPath('css/[name].[chunkhash].min.css').replace(
          'css/js',
          'css'
        ) //.js文件中的.css|.less|.sass内容转换成转换成.css文件。 2017-08-09,添加hash，有利于资源管理
      },
